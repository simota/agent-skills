...
Reference for Navigator's `stealth` recipe. Covers TLS / JA3 / JA4 fingerprinting, behavioral humanization, residential proxy rotation, and major anti-bot system handling (Cloudflare, Akamai, PerimeterX, DataDome).
...
**Refuse** when ToS deny is explicit OR user cannot articulate authorization. Document refusal in `.navigator/refusal-log.md`.
...
Logging:
  Refusal/exception log: .navigator/refusal-log.md
  Request log: .navigator/stealth-runs/<date>.jsonl
```
