...
- OUT of scope: original article authoring (`note`/`zenn`/`qiita`/`devto`), SEO duplicate-content audit (delegate to `growth`), social ad creative (different domain), video cuts and thumbnails (delegate to `cue`).
