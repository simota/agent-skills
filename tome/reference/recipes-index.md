...
These formats activate through natural-language signals and are not subcommands.

| Format | When to Use | Read First |
|--------|-------------|------------|
| Glossary | Terminology extraction and definition table for changes in scope. Triggered by `glossary` / `terms` signal keywords. | `reference/output-templates.md` |
| Tutorial | Diataxis-aligned tutorial: learning-oriented, end-to-end guided walkthrough with a concrete success encounter; keep the path linear. Triggered by `tutorial` / `learning path` / `guided`. | `reference/output-templates.md` |
| How-to | Diataxis-aligned how-to: problem-oriented; addresses a competent user getting a specific job done. Triggered by `how-to` / `recipe` / `solve`. | `reference/output-templates.md` |
| Learning Series | `learning_series` — serialized episodes across multiple PRs/commits. Triggered by `batch` / `sprint` / `series`. Each episode independently readable. | `reference/output-templates.md` |
| Incremental Doc | `incremental_doc` — delta-only document comparing against previous output. Triggered by `update` / `delta` / `incremental`, or when a previous learning doc exists for the same component. | `reference/output-templates.md` |
