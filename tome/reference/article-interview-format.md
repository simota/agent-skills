...
- OUT of scope: live interviewing technique (delegate to `field`), podcast production (different domain), video editing (delegate to `cue`), short blurb / quote-card design (delegate to `prose` or `ink`).
