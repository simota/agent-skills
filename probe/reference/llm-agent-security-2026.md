...
- Pin model identifiers (e.g. `gpt-5.5-<snapshot>`, `claude-opus-4-8@<snapshot>`) — silent model upgrades invalidate prior reports. #TODO(agent): confirm current dated-snapshot ID format for both providers before citing a specific date/hash.
