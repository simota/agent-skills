...
- **canon regulatory recipes**: after scoping, Canon continues into audit trails, ROC/SAQ preparation, Policy-as-Code mapping, continuous-control specifications, and QSA coordination support.
...
PRESENT   →  delegate: Cloak (tokenization/masking), Sentinel (PAN scan, weak crypto),
             Probe (Req 11 scans), Canon `audit` (ROC/SAQ readiness), Crypt (key management)
```
...
- **To Canon `audit` / `policy`**: ROC/SAQ preparation, Policy-as-Code design, AOC artifact storage, QSA coordination support, and TPSP register (`Req 12.8`).
