...
- **Canon `eula`**: software-license review — grant scope, restrictions, IP, warranty, indemnity, termination, and audit.
- **Canon `tos`**: online-service terms. EULA governs installed or licensed software; ToS governs a service. A downloaded SDK or on-prem component may require both.
- **Canon `dpa`**: data-processing terms. EULA covers software rights; DPA covers personal data processed through the software.
- **Canon `privacy`**: data-subject disclosure. EULA touches privacy only at telemetry and usage-data clauses.
- **Canon `legal-gap`**: cross-document consistency. Use it to compare EULA IP ownership with the ToS user-content license.
...
- **Canon `gdpr` / `wcag` (follow-up recipes)**: standards audit. EULA review checks the contract; a follow-up recipe checks implementation evidence. OSS dependency license scanning uses a dedicated license-audit owner when available.
...
> **Canonical ToS-overlap clauses** (T-12 governing law, T-13 dispute resolution, T-15 severability, T-08 limitation of liability, T-06 IP ownership) live in `legal-document-checklists.md`. This table covers **EULA-specific software-license clauses only**. Treat the canonical T-08/T-12/T-13 rows as primary for generic liability and venue.
...
| Export controls / sanctions | Canon `regulatory` with EAR / OFAC / EU dual-use scope | Missing entirely | Medium |
| Telemetry / usage data | What the software collects | Privacy policy reference missing | Medium |

(For governing law and venue, use `legal-document-checklists.md` T-12 / T-13 as canonical; this file's "Jurisdiction-Specific Differences" section covers EULA-specific extensions.)
...
- **Within Canon**: run `regulatory` to map the EULA to SOC 2 license-management controls, ISO 27001 A.5.32, or an EAR/OFAC program; preserve the EULA finding IDs as evidence inputs.
