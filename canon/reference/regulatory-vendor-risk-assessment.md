...
- **Canon `vendor`**: vendor risk program design — inventory, tier policy, contract requirements, questionnaire process, SOC 2 report review, monitoring cadence, findings tracking.
- **Crypt (elsewhere)**: cryptographic architecture — asked when reviewing a vendor's encryption claims or key management design; Canon consumes the verdict.
- **Vigil (elsewhere)**: threat-hunt a vendor after a compromise; Canon runs the program that catches it earlier.
