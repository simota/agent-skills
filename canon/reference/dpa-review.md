...
- **Canon `dpa`**: GDPR Art. 28-aligned DPA review — controller/processor obligations, sub-processor approval, SCC module selection, TIA, audit rights, and breach notification SLA.
- **Canon `privacy`**: data-subject-facing privacy policy review. DPA is the back-stage B2B contract; `privacy` is the front-stage disclosure.
- **Canon `tos`**: service terms between provider and end user. DPA sits beneath ToS when a controller procures the service from a processor.
- **Canon `legal-gap`**: cross-document consistency. Use it after `dpa` to compare the DPA, privacy policy, and main service agreement.
- **Cloak**: implements consent flows, PII tagging, and data-flow code. Canon reviews the paper contract and hands implementation gaps to Cloak.
- **Canon `gdpr`**: codebase-level GDPR control audit. Cross-reference it when the contract promises controls that require implementation evidence.
...
> **Canonical privacy clauses** (P-01–P-14 generic privacy, G-01–G-08 GDPR data-subject-facing) live in `legal-document-checklists.md`. This table covers **DPA-specific Art. 28 controller-processor clauses only**. Walk both during SCAN: the canonical list for data-subject disclosures, this list for controller-processor obligations.
...
(For the international-transfer safeguards clause itself, see `legal-document-checklists.md` G-08. This file's "Transfer Mechanism Selection" section covers the DPA-specific module-selection extension.)
...
- **Within Canon**: run `regulatory` to map the DPA to SOC 2 CC9.1, ISO 27001 A.5.19-A.5.22, or HIPAA BAA controls; preserve the DPA finding IDs as evidence inputs.
