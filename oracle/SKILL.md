...
| Cost | Budget alert `>120%`; wasted-token target `<5%`; route to the cheapest adequate model and consider cascade routing (escalate on low confidence); semantic cache similarity `>=0.8` with hit-rate target `>=60%`; prompt caching with a static prefix first. Combined techniques reach 70-90% savings |
| Agent design | Prefer custom agents under `3k` tokens; `25k+` needs redesign; measure compounding layer failure (95% per layer is 77% at five layers); design MCP tools as domain-aware actions, not generic CRUD, and keep tool descriptions under `2KB` with the most important usage context front-loaded |
