...
Purpose: Design and audit the `accessibilityIdentifier` taxonomy that every Snap query depends on. Identifiers are the contract between the shipping Swift code and the test suite — get the taxonomy right once, and refactors, localization, and copy edits stop breaking tests.
...
- **Owner**: Native for retrofit; Snap for label-fallback exceptions that are acceptable (system-provided UI Snap cannot annotate).
