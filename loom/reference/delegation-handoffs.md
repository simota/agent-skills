...
**Purpose:** Per-component handoff specs and the project routing-map format. Loom designs; these specs hand each piece to its owner.
...
Use the canonical envelope in `_common/HANDOFF.md`; the fields below are the Loom-specific payload each owner receives.
...
### Loom → Sigil (`LOOM_TO_SIGIL_HANDOFF`) — author skill bodies
```yaml
skill_specs:
...
```
Loom states *what* the skill must do and *which conventions* to mirror; Sigil decides Micro/Full and writes the body, scores it, and installs.

### Loom → Nexus (`LOOM_TO_NEXUS_HANDOFF`) — register routing + chains
```yaml
nexus_registration:
...
### Loom → Orbit (`LOOM_TO_ORBIT_HANDOFF`) — autonomous loop specs
```yaml
loop_spec:
...
### Loom → Latch (`LOOM_TO_LATCH_HANDOFF`) — enforcement hooks
```yaml
hook_specs:
  - event: PreToolUse
    matcher: "git commit"
...
### Loom → Grove (`LOOM_TO_GROVE_HANDOFF`) — placement
```yaml
placement:
