...
SURVEY found: schema changes need a migration + model update + test, repo-specific. Mechanism: recipe whose first step calls a **new** project skill `new-prisma-migration`. DELEGATE: Sigil authors the skill body; Loom registers the recipe with Nexus referencing it.
...
- **Embedding step logic Loom should delegate.** Loom names the step and its owner; the owning skill/agent holds the how.
