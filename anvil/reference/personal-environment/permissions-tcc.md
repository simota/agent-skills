...
5. **Unattended runs need pre-granted consent.** A launchd/cron/login-item automation cannot answer a prompt — the permission must already be granted (or, in managed fleets, pre-approved via MDM PPPC profiles, which is out of Anvil's scope to deploy).
