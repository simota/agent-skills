...
Purpose: Read this when organizing shell functions and aliases, managing `PATH`, wiring per-project tooling with `direnv` / `mise` / `asdf` / `nvm`, enforcing XDG directories, and lazy-loading heavy completions. Anvil owns **your** personal shell function layout; publishable CLI tools and team tooling manifests belong elsewhere.

## Scope Boundary

- **Anvil `shellfn`**: personal shell functions, aliases, env-var strategy, PATH hygiene, `direnv` / `mise` / `asdf` / `nvm` wiring, XDG migration, lazy-loading completions.
- **Anvil (elsewhere)**: publishable CLI tools — when a function grows flags / help text / tests / release needs, hand it off.
- **Hone (elsewhere)**: Claude Code hooks that invoke shell commands — Anvil provides the env, Hone wires the trigger.
...
- PATH/env audit reveals secrets or risky eval in startup → `Sentinel` + Anvil `audit`.
- Reproducing env on a new machine → Anvil dotfile-manager recipe (stow / chezmoi).
