...
> Apple's design guidelines reference for the Dock skill. macOS 26 "Tahoe" / Liquid Glass-aware.
...
→ Implementation: `SKILL.md` Core Contract "Liquid Glass scope" row, `reference/layout-patterns.md`, `reference/modern-stack.md`.
...
- **Cursor rects**: change the pointer shape (`NSCursor.pointingHand`, `.resizeLeftRight`, `.iBeam`, etc.) over draggable dividers, resize handles, and clickable non-button elements. In SwiftUI, use `.onHover { NSCursor... }` or `.pointerStyle()` (macOS 13+ where available) rather than leaving the default arrow over every control.
...
→ Implementation: `reference/distribution.md` (packaging), `reference/modern-stack.md` (Xcode 26 toolchain).
