...
> Scope: App Sandbox (`com.apple.security.*` sandbox keys) and TCC (Info.plist usage-description strings + user-approved system settings). Signing/notarization mechanics live in `reference/xcrun-cli.md`; the ship decision matrix lives in `reference/distribution.md`.
...
| `com.apple.security.automation.apple-events` | Send Apple Events broadly (paired with per-target `NSAppleEventsUsageDescription` prompts at runtime) | Only for automation-hub apps; route "Dock builds a scriptable app" vs "app scripts other apps" through the Dock↔Wield boundary — this entitlement is for the latter direction |
...
| Direct `/dev` or low-level IOKit access outside declared device categories | Sandbox restricts IOKit user-client connections to declared categories (camera/USB/Bluetooth) | Move the driver-adjacent logic into a System Extension / DriverKit component (out of Dock's scope — pointer only) |
...
- Cross-reference: `reference/distribution.md` (hardened runtime + notarization flow), `reference/xcrun-cli.md` (`codesign --entitlements` inspection), `reference/xpc-helpers.md` (app-group-backed IPC)
