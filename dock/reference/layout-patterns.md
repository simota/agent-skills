...
- **Chrome only** — never apply `.glassEffect()`/`GlassEffectContainer` to content views (document body, list rows, canvas). This mirrors the SKILL-level Liquid Glass scope rule (see `dock/SKILL.md` Core Contract and `reference/mac-hig.md`).
