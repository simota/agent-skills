...
Production reference for taking a macOS app from build to a user's Dock: channel decision, code-signing chain, hardened runtime, notarization flow, packaging, self-updating (Sparkle), and Mac App Store submission. Exact command flags live in `reference/xcrun-cli.md` — this file owns the *decision* and *flow* narrative; do not duplicate flag tables here.
...
- Certificate discovery/import for CI: `reference/xcrun-cli.md` § 5 (`security` keychain commands).
...
- Exact `codesign` flags (`-s`, `--options runtime`, `--entitlements`, `-dv --verbose=4`) → `reference/xcrun-cli.md` § 2.
...
- Exact `notarytool`/`stapler`/`spctl` flags and the `store-credentials` keychain-profile setup → `reference/xcrun-cli.md` § 5.
...
- Exact `hdiutil`/`pkgbuild`/`productbuild`/`productsign` invocations → `reference/xcrun-cli.md` § 6.
...
| Shipping `com.apple.security.get-task-allow = true` in a release build | Notarization rejects the submission outright | Strip debug entitlements from the release `.entitlements` file, verify via `codesign --display --entitlements -` (`reference/xcrun-cli.md` § 3) |
...
- [ ] Release artifact handed to `Launch` (`DOCK_TO_LAUNCH_HANDOFF` — `reference/handoffs.md`) with distribution channel, notarization status, and appcast coordination noted
...
- Cross-reference: `reference/xcrun-cli.md` (exact `codesign`/`notarytool`/`pkgbuild`/`productbuild` command flags + CI recipe), `reference/sandbox-entitlements.md` (entitlement catalog + hardened-runtime-vs-sandbox distinction), `reference/xpc-helpers.md` (helper-tool lifecycle referenced by Sparkle's XPC services)
