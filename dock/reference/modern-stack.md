...
**Format model:** Structured like [`native/reference/modern-stack.md`](../../native/reference/modern-stack.md) (read first for the iOS/Android baseline and shared Swift 6.3 concurrency model — not duplicated here). Scope here is macOS only.

> Out of scope: Mac Catalyst stack specifics (see `reference/catalyst-decision.md`), iOS/Android (see `native/reference/modern-stack.md`).
...
- **Swift 6.3** carries forward Swift 6.2's Approachable Concurrency model (default `MainActor` isolation for new targets, `@concurrent` for explicit background work) — see `native/reference/modern-stack.md` §Swift 6.2 Approachable Concurrency for the full mechanism; it applies identically on macOS targets.
...
| Sidebar/detail/inspector 3-column | `NavigationSplitView` | macOS 13+ | Do not nest inside `NavigationStack` — same forbidden pattern as iOS (`native/reference/modern-stack.md`) |
...
| Observation | `@Observable` macro | macOS 14+ | Same 2026-default guidance as iOS — see `native/reference/modern-stack.md` §Observation framework |
...
|-----------|-----|----------------------|
| **SwiftData** (macOS 14+) | Default for new SwiftUI-centric apps — same 2026 production-ready guidance as iOS (`native/reference/modern-stack.md` §SwiftData vs Core Data) | Store lives in app container; CloudKit sync requires iCloud entitlement + container identifier |
...
|------|-----------|-------|
| HTTP / API calls | `URLSession` + async/await | Identical API surface to iOS (`native/reference/modern-stack.md` §Library quick reference) |
...
| Work that must run even when the app itself isn't launched | Launch Agents / Launch Daemons via `SMAppService` (macOS 13+) | See `reference/xpc-helpers.md` for registration lifecycle — Dock's `SKILL.md` Always list mandates `SMAppService` over legacy `SMJobBless`/manual `launchctl` |
...
Shared with iOS — see `native/reference/modern-stack.md` §Swift 6.2 Approachable Concurrency for the full model (default `MainActor` isolation, `@concurrent` for explicit background work, caller-isolation inheritance). No Mac-specific concurrency divergence: the same `@Observable` + `@MainActor`-by-default + `@concurrent` pattern applies to macOS targets identically.
...
- Sibling reference: [`native/reference/modern-stack.md`](../../native/reference/modern-stack.md) — shared Swift 6.3 concurrency model, `@Observable` pattern, SwiftData/Core Data trade-off (not duplicated here)
