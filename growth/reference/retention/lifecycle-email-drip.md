...
- **growth `lifecycle-email`**: Full lifecycle email drip design (this document).
- **growth `reengagement` (elsewhere)**: Dormant re-engagement tactics. Lifecycle includes but is broader than re-engagement.
- **growth `winback` (elsewhere)**: Cancelled/long-dormant recovery. Lifecycle is active-user nurture; winback is recovery.
- **Prose `notification` (elsewhere)**: Copy authoring per email. Lifecycle designs the *plan*; Prose writes the *words*.
- **gateway (elsewhere)**: Email delivery infrastructure.
...
- gateway: delivery, tracking, bounce handling
...
- **Handoffs**: Prose, gateway, Pulse, Experiment, Cloak/Canon[regulatory], Scaffold.
