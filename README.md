...
| **Xquik** | _"Every X workflow starts with a verified boundary."_ - X/Twitter reads, monitors, webhooks, agent integrations, and explicitly approved account actions through documented Xquik surfaces | Source packets, monitor plans, integration packets, action receipts |

**Schema > Stream chain**: Schema (data model) > Stream (pipeline design)
**Xquik > Relay > Stream chain**: Xquik (X source or monitor) > Relay (event delivery) > Stream (downstream pipeline)
...
├── xquik/SKILL.md      # X/Twitter data and automation through Xquik
...
##### X Brand Monitoring (Xquik)

```
/Xquik monitor
Design a bounded X monitor for our brand and product names.
Route verified events to our webhook without taking account actions.
```

**Output**: Monitoring packet with filters, event contract, deduplication, verification, retention, and stop conditions
