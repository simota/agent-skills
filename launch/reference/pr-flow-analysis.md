...
- **launch `pr-flow`**: distributional and risk-focused PR analytics. Histograms, percentiles, Lorenz, large-PR flags.
- **launch `weekly` / `monthly` (elsewhere)**: aggregate counts and category breakdowns without distribution analysis.
- **launch `dora` (elsewhere)**: DORA 5-metric percentile-band classification (DORA 2025). Lead Time at the DORA level; prstats decomposes into Pickup/Review/Merge phases.
- **launch `okr` (elsewhere)**: outcome linkage; prstats is purely flow-statistics.
...
- **Launch release-planning recipes**: consume `pr-flow` output to size release windows.
...
Hand off to Pulse for live rendering; Launch produces the spec and seed data.
...
- Treating P90 as an SLO target without team agreement — Launch reports the number; the team and Beacon set targets. Reporting an unagreed target is policy creep.
