...
- **launch `okr`**: PR-to-Objective mapping and KR narrative for a quarter or OKR window. Outcome-vs-output framing.
- **launch `weekly` / `monthly` (elsewhere)**: time-window aggregation without Objective linkage.
- **launch `notes` / `changelog` (elsewhere)**: changelog by release, not by Objective.
- **launch `retro` (elsewhere)**: sprint narrative; OKR is quarterly and outcome-focused.
- **Pulse (elsewhere)**: KPI dashboard. Pulse owns ongoing KR tracking; Launch produces the linkage report.
...
- **Launch release-planning recipes**: may map releases to KRs; only the `okr` recipe scores Objective health.
- **Guardian (elsewhere)**: PR strategy. Guardian sees PRs as merge candidates; Launch sees them as KR contributions.
...
- Output-vs-outcome confusion — "ship feature X" is output. "X-driven retention up 5pp" is outcome. Launch reports must keep the distinction visible; if the team only has output KRs, flag it explicitly.
...
- **To Pulse**: live KR progress dashboard. Pulse owns the running tracker; Launch produces the quarterly snapshot and orphan analysis.
- **To Beacon**: KRs framed as reliability targets (SLO uptime, MTTR ceiling). Beacon owns SLO design; Launch reports OKR-side health.
- **To Launch**: KRs gated by releases. Launch coordinates release-to-KR alignment; Launch reports the linkage.
