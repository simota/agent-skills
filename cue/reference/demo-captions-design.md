...
- **cue `captions`**: Caption authoring for demo videos (this document).
- **cue `voiceover` (elsewhere)**: TTS narration (captions derive from narration text).
- **cue `demo` / `record` (elsewhere)**: Recording orchestration.
...
```
INPUT        →  narration text from cue `voiceover`
...
HANDOFF      →  cue `voiceover`: narration sync
...
- **Handoffs**: cue voiceover, cue thumbnail, polyglot, Builder.
