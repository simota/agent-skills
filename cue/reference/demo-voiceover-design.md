...
- **cue `voiceover`**: TTS narration for demo videos (this document).
- **cue `demo` / `record` / `scenario` / `onboard` (elsewhere)**: Scenario + recording orchestration.
- **cue `captions` (elsewhere)**: On-screen captions (complements voiceover).
...
**Default**: -14 LUFS for YouTube + LinkedIn (the two primary demo destinations in 2026). -16 LUFS for self-host / Vimeo / docs. Verify True Peak ≤ -1 dBTP to prevent clipping on transcode. Perceptual quality verification via `reference/demo-quality-metrics.md`.
...
```
SCRIPT       →  receive scenario (from cue `scenario`)
...
ACCESS       →  generate caption track (hand off to cue `captions`)
             →  AD track if visual-only content exists
             →  WCAG 1.2.5 compliance

DELIVER      →  MP4 (H.264 + AAC 192kbps) per platform

HANDOFF      →  cue `captions`: caption generation
             →  cue `thumbnail`: thumbnail design
...
### Accessibility
- Captions: [auto / manual; hand off to cue `captions`]
- Audio Description: [yes / no]
- WCAG 1.2.5 compliance: [pass / needs work]

### Handoffs
- cue `captions`: SRT / WebVTT
- cue `thumbnail`: thumbnail
- Builder: build-pipeline integration
```
...
- **Handoffs**: cue captions, cue thumbnail, Builder.
