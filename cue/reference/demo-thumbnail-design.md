...
- **cue `thumbnail`**: Thumbnail design for demos (this document).
- **cue `demo` / `voiceover` / `captions` (elsewhere)**: Video production phases.
...
HANDOFF      →  growth: CTR tracking + A/B results
             →  cue `voiceover` / `captions`: final bundle
...
- **Handoffs**: growth, cue bundle, Builder, funnel.
