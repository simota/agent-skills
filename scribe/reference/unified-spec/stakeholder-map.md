...
Purpose: Map stakeholders on a Power × Interest grid, assign an engagement mode per quadrant, and design communication cadence and information-flow patterns. Scribe uses this to anchor the L0 Vision audience and to drive communication plans for L1-L3 review cycles.

## Scope Boundary

- **scribe `stakeholder`**: Power/Interest grid + engagement design (this document).
- **scribe `vision` (elsewhere)**: L0 Vision Block — stakeholder map informs the audience section.
- **scribe `raci` (elsewhere)**: Per-item responsibility assignment (different question).
...
This dovetails with `scribe raci`.
...
### Handoffs
- Scribe L0 Vision: audience section references this map
- Scribe `raci`: per-item responsibility assignment
...
- **Handoffs**: Scribe L0, Scribe `raci`, Prose, Canvas, Scribe.
