...
- OUT of scope: visual design (delegate to `vision`), full design-system tokens (`muse`), animation (`flow`), per-platform bios (`brand bio`), Topic DNA / positioning (`brand topic-dna`), conversion funnel for paid offers (`funnel`), SEO meta (`growth`).
...
- Handoff briefs prepared for `vision`, `muse`, `brand bio`, and `growth`.
