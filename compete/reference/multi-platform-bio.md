...
- OUT of scope: positioning statement / Topic DNA (`brand topic-dna`), portfolio site IA (`brand portfolio`), full LinkedIn or GitHub profile structure (`brand profile`), CFP authoring (`brand content`), microcopy / error strings (delegate to `prose`).

## Core Concepts

### One Canonical Source

All bios derive from one **Master Positioning Statement** (the output of `brand topic-dna`). Without a single source, bios drift over months and visitors meet a different person on each platform.
...
1. **Get master positioning** from `brand topic-dna` output.
