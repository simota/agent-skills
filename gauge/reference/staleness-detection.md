...
# current is claude-opus-4-8 / claude-sonnet-5 / claude-haiku-4-5-20251001 / gpt-5.6 / Gemini 3.5).
grep -rln -E 'claude-3-5-sonnet|claude-3-opus|claude-instant|gpt-4-turbo|text-davinci-00[0-9]|claude-(opus|sonnet)-4-[0-7]\b|claude-haiku-3(-[0-9]+)?\b|claude-haiku-4-[0-4]\b|gemini-1\.[0-9]|gemini-2\.[0-9]' SKILL_GLOBS
...
Verification: vendor-managed model identifiers move fast. Flag any bare pre-current-generation model-id reference — including `claude-sonnet-4-*`, `claude-opus-4-4` through `4-7`, `claude-haiku-3-*` / `claude-haiku-4-0` through `4-4`, a bare `gpt-4o` chat-model mention, and `gemini-1.*` / `gemini-2.*`; the fix is to either annotate as historical or replace with the current family reference (e.g. `claude-opus-4-8`, `claude-sonnet-5`, `claude-haiku-4-5-20251001`, `gpt-5.6`, `Gemini 3.5 Flash`). Do NOT flag `gpt-4o-transcribe`, `gpt-4o-mini-transcribe`, `gpt-4o-audio-*`, `gpt-4o-realtime-*`, `gpt-4o-search-*`, or `gpt-4o-mini-tts` — those are standing product names, not a superseded chat-model version. Current generation itself (`claude-opus-4-8`, `claude-sonnet-5`, `claude-haiku-4-5-20251001`) must never be flagged.
