# Vector Execution Templates

Detailed templates for each phase of Vector's execution process.
...
### Screenshots
- Initial state: `.vector/screenshots/recon_initial.png`
```
...
```typescript
// Screenshot
await page.screenshot({ path: '.vector/screenshots/result.png', fullPage: true });

// HAR (Network log)
const context = await browser.newContext({ recordHar: { path: '.vector/har/session.har' } });

// Console logs
page.on('console', msg => {
  fs.appendFileSync('.vector/logs/console.log', `[${msg.type()}] ${msg.text()}\n`);
});
```
...
|------|------|---------|------|
| JSON | `.vector/data/[name].json` | [N] | [X KB] |
| CSV | `.vector/data/[name].csv` | [N] | [X KB] |

### Screenshots
| Name | Path | Description |
|------|------|-------------|
| [name] | `.vector/screenshots/[name].png` | [What it shows] |

### Network Logs
- HAR file: `.vector/har/[session].har`
- Request count: [N]
- Errors: [N]

### Console Logs
- Log file: `.vector/logs/console.log`
- Errors: [N]
- Warnings: [N]
...
| Type | Path |
|------|------|
| Screenshots | `.vector/screenshots/` |
| HAR logs | `.vector/har/` |
| Console logs | `.vector/logs/` |
