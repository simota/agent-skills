# Vector Data Extraction Patterns
...
async function collectPaginatedData(page: Page): Promise<any[]> {
  ...
  while (hasNextPage) {
    ...
    await page.screenshot({
      path: `.vector/screenshots/page_${pageNum}.png`
    });
...
async function submitForm(page: Page, formSelector: string): Promise<SubmitResult> {
  ...
  const preSubmitScreenshot = `.vector/screenshots/form_pre_submit.png`;
  ...
  // Capture result
  const postSubmitScreenshot = `.vector/screenshots/form_post_submit.png`;
