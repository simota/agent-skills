...
Reference for Vector's `parallel` recipe. Covers `BrowserContext` isolation, worker pool sizing, shared auth state, per-task timeouts, queue management for batches >100 URLs, and throughput vs detection trade-off.
...
const CHECKPOINT = '.vector/checkpoint.jsonl';
