...
- OUT of scope: image generation itself (`image`), prompt construction (`image-prompt` / `image-cinematic`), provenance / disclosure (`image-provenance`), post-processing (`image-postprocess`), legal contract review (delegate to `canon`).
