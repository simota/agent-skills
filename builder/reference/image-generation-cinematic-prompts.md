...
- OUT of scope: text-to-image API integration (`image`), seed-locked variation (`image-batch`), style-transfer / character consistency (`image-style`), upscaling / inpainting (`image-postprocess`), provenance / disclosure (`image-provenance`), content policy (`image-policy`).
