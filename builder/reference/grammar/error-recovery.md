...
- **Builder `error`**: recovery strategy + diagnostic template design (what to show, when to resync, how to point at source).
- **Builder `parser`**: grammar-class + generator choice (recovery strategy is a factor but not the focus).
- **Builder `lexer`**: token-level error states (invalid byte, unterminated string) — cross-reference, not duplicate.
