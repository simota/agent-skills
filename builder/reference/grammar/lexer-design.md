...
- **Builder `lexer`**: whether to extract tokenization as a distinct stage, and how to structure it (modes, context-sensitivity, trivia, lookahead).
- **Builder `parser`**: grammar class + parser generator for the full syntactic layer (LL/LR/PEG/GLR, ANTLR vs tree-sitter vs hand-written RD, etc.).
- **Builder `error`**: recovery strategy; lexer-level recovery (invalid-byte token, unterminated string) is scoped here but diagnostic templating is `error`.
