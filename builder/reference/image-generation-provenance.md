...
- OUT of scope: image generation itself (`image`), prompt design (`image-prompt` / `image-cinematic`), post-processing (`image-postprocess`), policy refusals (`image-policy`), copyright case-by-case legal review (delegate to `canon`).
...
|----------------|---------|
| `claim_generator` | Tool that created the manifest (e.g., `nano-banana-2`) |
...
| Government takedown (regulatory) | Engage `canon` regulatory review; provide C2PA chain |
...
Builder's `image-policy` Recipe handles refusals; `image-provenance` documents what is permitted.
