...
- **Builder `pkg`**: packaging manifests, cross-compile matrix, signing/attestation, install scripts, update-checker design, distribution-channel selection.
- **Builder (elsewhere)**: the business logic inside the CLI (packaging does not change behaviour).
- **Hone `env`**: user-side install into their own dotfiles environment (`brew install` in a bootstrap workflow).
