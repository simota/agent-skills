...
- **builder `image-postprocess`**: upscaler choice, mask authoring, inpainting prompts, artifact detection heuristics, export format tradeoffs (WebP / AVIF / PNG / JPEG).
- **builder `image` / `image-batch` (siblings)**: produce the base raster. Do not upscale at generation time — render at native ceiling first (1K / 2K / 4K), then post-process if needed.
...
If the issue is "image is too small" → `image-postprocess`. If "wrong subject" → re-run `image` with a new prompt, not inpainting.
...
- style pivots ("make it anime now") — that is an `image` call, not `image-postprocess`
