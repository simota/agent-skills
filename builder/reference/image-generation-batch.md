...
- **builder `image-batch`**: batch Python script, seed stability plan, parallel API calls with rate limits, output naming, perceptual-hash dedup, `metadata.json` per asset.
- **builder `image-style` (sibling)**: defines the style anchor (reference image / token set) consumed by `image-batch`. Call `image-style` first if the style is not yet locked.
- **Upstream caller (elsewhere)**: asset brief, count, resolution, and naming spec — `image-batch` does not invent the list.
- **Vitrine (elsewhere)**: catalog the produced set after `image-batch` completes.

If the brief is "one hero image, try three variants" → default `image`. If it is "120 card faces, consistent frame and lighting" → `image-batch`.
...
- Run `image-batch` before `image-style` has locked the anchor — early assets diverge and the later ones cannot reconcile.
