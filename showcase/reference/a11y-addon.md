...
- OUT of scope: full WCAG 2.2 audit (delegate to `palette` `a11y` subcommand or `canon`), production-runtime a11y (`palette`), screen-reader testing (manual), color-token contrast design (`muse`), legal compliance (`comply` / `clause`), focus animation (`flow`).
