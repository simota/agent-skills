...
Record the engine set per cluster. Per-engine wording variants are preserved — Flux[ideate] often surfaces them in dialogue as "Codex framed this as X, Claude framed it as Y — does either resonate more?".
...
Flux[ideate] supports two synthesis outputs depending on invocation:
...
- **Idea cards** ordered: `VERIFIED-DIVERGENT` first (lead with breakthroughs), then `LIKELY`, then `UNIVERSAL` (Flux[ideate]'s inversion vs Spark)
- Each card: id, idea_text, engine-attribution tag, rationale (one line), connection_to_theme
- **Flux[ideate]'s framing question** — one open question per shipped idea, in Flux[ideate]'s voice, that pulls the user into reacting (per the Turn Structure: Receive → Challenge → Prompt)
- **Round close** — a "which 1-3 want to go deeper on?" prompt routing the next dialogue turn

The user's pick becomes the **seed for the next dialogue round** — not a multi round, but a normal Flux[ideate] turn drilling into the selected idea.
...
- **Diamond reading** — Flux[ideate]'s interpretation of which mode the matrix suggests is highest leverage right now (e.g., "PROPOSE column is thin — the theme isn't concrete enough yet; recommend continuing EXPAND")
...
`multi` is **one turn** inside a larger Flux[ideate] dialogue. The next turn behavior:

| User reaction to multi output | Flux[ideate]'s next turn |
|-------------------------------|-------------------|
| Picks 1 idea | Drop multi mode; return to normal Flux[ideate] dialogue with the picked idea as the new active theme; continue with the appropriate mode |
| Picks 2-3 ideas | Use `propose` mode in normal Flux[ideate] to weave them into a composite; OR offer SCAMPER `combine` lens |
| Picks all / can't pick | Flux[ideate] diagnoses paralysis — switch to `evaluate` mode in normal dialogue, surface the decisive axis |
...
The Flux[ideate] main context **tracks dialogue state across rounds**, so the duplicate-of-prior-turn check at GROUND step has data to work with.
