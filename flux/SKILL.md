...
- **Baseline (2026-05)**: Claude + Codex (dual-engine). agy joins as third axis when AVAILABLE — agy's Gemini model-priors (Gemini 3.6 Flash (High), High effort tier) + 1M-context analogy uplift is larger for Flux than for other Pattern D skills (Gemini 3.6 Flash (High) mandated — `_common/CLI_COMPATIBILITY.md §4 ‡`; no Deep Think).
