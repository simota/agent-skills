...
- **Gear[gha] `cache`**: GitHub Actions-specific caching — `actions/cache`, built-in `setup-*` caches, Docker layer cache via `type=gha`, key/`restore-keys` design, 10 GB / 7-day eviction.
