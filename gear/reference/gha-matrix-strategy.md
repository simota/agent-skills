...
- **Gear[gha] `matrix`**: GitHub-Actions-specific matrix mechanics (`strategy.matrix`, `include` / `exclude`, `fail-fast`, `max-parallel`, `fromJSON` dynamic matrix).
- **Gear `ci`**: provider-agnostic CI/CD posture — which provider, which stages, which quality gates. If the question is "should we test on 3 Node versions or 2?", stay in Gear[gha]. If it's "should we use GHA or Buildkite for this matrix?", route to Gear `ci`.
