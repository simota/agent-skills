...
- **Gear[gha] `secret`**: GitHub Actions secret surface — OIDC federation, env vs repo secrets, `vars` vs `secrets`, `::add-mask::`, `pull_request` vs `pull_request_target` fork-PR isolation, reusable-workflow secret scoping.
- **Gear `secret`**: application-layer secret management — Vault, AWS Secrets Manager, Doppler, sealed-secrets, SOPS, runtime secret rotation. Complementary: Gear[gha] `secret` delivers the **short-lived cloud token** to the runner; Gear `secret` is where the app reads database passwords and API keys at runtime.
- **Sentinel**: static code scanning for hardcoded secrets. Complementary: Gear[gha] `secret` designs the architecture so secrets never need to be in code; Sentinel catches the cases where a developer pasted one anyway.
