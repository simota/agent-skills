...
**BP-11 note**: filesystem-as-memory (RP-7) protects **across** iterations (fresh context at each start) and `CONTEXT_OVERFLOW`/memory-pointer externalizes tool **outputs** > 1KB — neither protects a single long iteration that exhausts context **within** the iteration before reaching its commit/exit. Server-side compaction (Anthropic beta header `anthropic-beta: compact-2026-01-12`, default trigger 150K input tokens, min 50K; supported on `claude-fable-5`, `claude-opus-5`, `claude-sonnet-5` and prior Opus/Sonnet generations — verified 2026-07-25) proactively summarizes the running conversation transcript and is the recommended strategy for task-oriented prompts with heavy follow-up tool use. It is complementary to, not a substitute for, RP-7 and memory-pointer. Gotchas: compaction costs an extra sampling step that counts against rate limits and billing, and with server tools the trigger is re-checked at the start of *each* sampling iteration — a single request can compact more than once. Monitor `usage.iterations` to see when it fires. **Note:** with Opus 5's 1M-token default context window, a loop iteration reaching a 150K trigger is no longer the common failure mode — raise the trigger or rely on RP-7 before adding compaction. [Source: https://platform.claude.com/docs/en/build-with-claude/compaction]
