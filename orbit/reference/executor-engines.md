...
**Model — agy mandate (user policy, 2026-06-23)**: always **Gemini 3.6 Flash (High)** for every step/subagent (it is also agy's default, so "never switch away"); pin via `agy --model "Gemini 3.6 Flash (High)"` (v1.0.5+) or `/model` (TUI). Detail: `_common/CLI_COMPATIBILITY.md §4 ‡`. This OVERRIDES the generic "keep the default unless overridden" guidance above for the agy column.
