...
**Pattern type:** D (Divergence-primary). See `_common/MULTI_ENGINE_RECIPE.md §Pattern D`. This document specifies only the **Researcher-specific differences**.
...
## 1. SCOPE (Researcher-specific inputs)
...
## 2. FAN-OUT JSON output schema (Researcher-specific)
...
## 3. CLUSTER — identity rules (Researcher-specific)
...
## 4. SCORE — methodology coverage matrix (Pattern D, Researcher-specific)
...
## 5. GROUND — ethics / IRB / feasibility checks (Researcher main context, never delegated)

For every cluster (especially `CANDIDATE-DIVERGENT`), the Researcher main context verifies:
...
## 6. SYNTHESIZE — Combined Plan vs Portfolio (Researcher-specific merge)

Researcher supports two merge strategies. Default depends on triangulation density: **Combined Plan** when the surviving clusters form a coherent triangulation triple/quad; **Portfolio** when the clusters address independent research questions or compete on stance.
...
**Combined Plan output**: a single research plan matching the standard Researcher report structure (`## User Research Report` → `### Research Objective` → `### Methodology` → ...), with the multi-method composition surfaced explicitly under `### Methodology` and `engine_concurrence` front matter at the top.
...
```
You are the {engine} research-design subagent for Researcher.
...
The three subagents return JSON; Researcher main context handles NORMALIZE through PRESENT.
...
- `researcher/reference/interview-guide.md` — applied at SYNTHESIZE for interview-method clusters
- `researcher/reference/participant-screening.md` — applied at GROUND for recruitment/inclusion checks
- `researcher/reference/bias-checklist.md` — applied at GROUND for bias-floor checks
- `researcher/reference/ai-assisted-research.md` — applied at GROUND for AI-moderation / synthetic-user disclosure
- `researcher/reference/survey-quantitative-design.md` — applied at SYNTHESIZE for survey-method clusters
- `researcher/reference/continuous-discovery-mixed-methods.md` — applied at SYNTHESIZE for Combined-Plan triangulation logic
