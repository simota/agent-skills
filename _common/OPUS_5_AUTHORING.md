...
**Apply by (reviewers/detectors — Judge, Gauge, Sentinel, Radar, Attest, Canon, Probe, Matrix, Vigil, Chain):**
...
**Apply by:**
- Audit SKILL.md files for prohibition lists ("never…", "always…", "do not…") that encode a *rule* where the *outcome* would steer better, and convert them. Safety gates, destructive-action confirmations, and protocol contracts are exempt — they are constraints, not style guidance. When a negative must remain, pair it with the value it protects and the substitute action ("Do not X — because Y, instead do Z"); a bare prohibition names only what to avoid, not what to do, so it gets applied both too broadly and too narrowly.
- Treat few-shot blocks as a cost. Keep them where the output shape is genuinely non-obvious (schemas, protocol markers, handoff blocks); drop them where they merely illustrate judgment. Where kept, name which attributes to imitate and which to ignore (e.g., "match the information hierarchy; ignore the color and photography") — an unscoped example invites over-imitation of incidental details.
