...
|--------|---------|-------------|--------|
| `REQ-{NNN}` | Functional / business requirement (user-facing need) | scribe[unified] (`L1`), scribe (`FR`) | `REQ-001` |
| `CFR-{NNN}` | Cross-functional requirement (perf, security, a11y — formerly "NFR") | scribe[unified], scribe | `CFR-001` |
| `AC-{FEATURE}-{NNN}` | Acceptance criterion (the verifiable unit) | scribe[unified] (`L3`), attest, scribe | `AC-LOGIN-001` |
| `SC-{AC-id}-{type}-{NNN}` | BDD scenario derived from an AC (`type` ∈ HP/NP/BP/EP) | attest, scribe[unified] | `SC-AC-LOGIN-001-HP-001` |
...
| `DISQ-{NNN}` | Disqualification / hard-fail condition (scribe[unified] `L4`) | scribe[unified] | `DISQ-001` |

**Backward-compatibility note.** Legacy `NFR-*` (scribe) maps to `CFR-*`; legacy bare `US`
(scribe[unified] user story) maps to `REQ-*`. Prefer `CFR`/`REQ` in new packages; accept the legacy forms
on read.
...
When a spec package is durable enough to drive implementation (scribe[unified] `Full`/`Standard`,
...
```yaml
# .traceability.yaml — generated by scribe[unified]/scribe, verified by attest, gated by guardian/judge
version: 1
...
|-------|--------------------|
| `scribe[unified]` / `scribe` | **Author** `links[]` (REQ/CFR/AC/SC) at spec time; set initial `verdict: NOT_TESTED`. |
...
Coverage thresholds align with each skill's existing bars (scribe[unified] `Full ≥95%` / `Standard ≥85%`
