...
| **Anthropic Computer Use** | Screenshot + cursor / keyboard control via the Claude API or Claude Code Q1 2026 integration | Claude (Sonnet 5 / Opus 5) | GA contract is the API tool + Cowork / Claude Code desktop. Use for cross-app journeys that exit the browser (file system, native dialogs). Pair with prompt-injection scanning. |
...
| **Maestro MCP** | Mobile only | Exposes Maestro to an MCP client (Claude Code etc.) so flows are authored and driven from the coding agent's session rather than hand-written YAML | Agent-authored flows still need the review gate below; MCP access does not lower the assertion bar | <https://verygood.ventures/blog/maestro-mcp-claude-mobile-ui-test-automation/> · <https://www.getpanto.ai/blog/maestro-mcp-mobile-test-automation> |

**Heal at authoring time, not at run time.** The durable version of self-healing is the agent picking a stable selector (accessibility identifier over text/coordinates) *while writing the test* — not a runtime engine silently substituting a locator on a suite that already shipped. Maestro's own framing of agentic testing is that "the agent fixes its own selector while it is writing the test, not while the test is running later" (<https://maestro.dev/insights/self-healing-tests-fixing-flaky-ui-automation>). Runtime healing remains useful as a signal — but a heal event is a **finding to triage**, not a pass; keep it surfaced in the report (see the review gate below).
