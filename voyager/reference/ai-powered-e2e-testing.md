...
| **Anthropic Computer Use** | Screenshot + cursor / keyboard control via the Claude API or Claude Code Q1 2026 integration | Claude (Sonnet 5 / Opus 4.8) | GA contract is the API tool + Cowork / Claude Code desktop. Use for cross-app journeys that exit the browser (file system, native dialogs). Pair with prompt-injection scanning. |
