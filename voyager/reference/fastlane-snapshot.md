...
Every additional device × language multiplies capture time and CI minutes linearly. Ask First (per Voyager[ios]'s Boundaries) once the matrix exceeds **3 devices × 3 languages**.
...
- Marketing frame text (`framefile.json`) is a Vision/Launch concern for copy; Voyager[ios]'s responsibility ends at producing clean, correctly-matrixed raw screenshots for `frameit` to consume.
