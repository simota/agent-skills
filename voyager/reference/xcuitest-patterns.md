...
```swift
// Only for canvases / system UI Voyager[ios] cannot annotate with identifiers
...
- `appstore`: fastlane snapshot wiring. Produces `Voyager[ios]file` (devices / languages), `Voyager[ios]shotHelper.swift` (drops into test target), a dedicated `Screenshots` scheme, and a status-bar override pre-script. Combine with frameit for marketing frames. Output goes to artifact store, never committed to main.
...
| `fastlane snapshot`, `Voyager[ios]file`, `Voyager[ios]shotHelper`, `App Store screenshot`, `frameit`, `marketing screenshot` | `appstore` |
...
- fastlane_snapshot_pipeline: Configure fastlane snapshot (Voyager[ios]file, Voyager[ios]shotHelper.swift, snapshot() calls in UI tests) for App Store screenshots across device matrix, languages, and orientations; combine with frameit for marketing frames
