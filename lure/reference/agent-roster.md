...
The complete delegate roster for the `lure` LP studio chain, organized by stage. Each entry names the role, the artifact returned to `lure`, and the overlap watch-out so `lure` does not double-book responsibilities.

`lure` itself never writes copy, designs pixels, or ships code. It selects delegates and brokers handoffs.
...
Parallel fan-out: yes (3 concurrent). Researcher is independent. Compete and Voice can both run alongside.
...
| Orchestrator | Overlap with `lure` | Differentiation |
|--------------|----------------------|------------------|
| `funnel` | Stage 4 (Structure) | Funnel is the LP-section specialist; lure is the chain. Lure invokes funnel. |
| `atelier` | Stage 5 (Design) | Atelier handles design pipeline for multi-artifact bundles; lure for single LP handles design directly. Lure invokes atelier on bundle requests. |
...
| `nexus` | Generic multi-domain | Lure is LP-axis only. If the request crosses LP axis (security + data + infra + LP), escalate to nexus with the LP slice as a sub-task. |

`lure` ≈ 30% capability overlap with the union of (funnel + atelier + portions of titan). The 70% unique surface is research-to-launch coordination, recipe selection, and quality-gate enforcement specific to landing pages.

---

## Axis Ownership Map

Six quality axes mapped to agent clusters. `lure` is the conductor; each axis has a lead and supporting cast. Rubrics live in `craft-standards.md` and `ia-blueprint.md`.
...
Pattern: Vision (decide) → Muse (encode) → Palette (humanize) → Frame/Ink/Sketch (asset) → Pixel (verify). `lure` scores Design Rubric ≥ 18/24 at DESIGN gate.
...
Pattern: Vision sets motion story → Muse tokenizes → Flow implements → Palette validates reduced-motion → Bolt enforces INP budget. `lure` scores Motion Rubric ≥ 15/20 at DESIGN gate.
...
Pattern: Vision + Saga + Compete jointly author the Brand System record at STRATEGY → Prose carries voice into copy → Vision validates visual coherence → `lure` runs Three-Channel Coherence Audit (visual / voice / experience) at VERIFY. Score Brand Rubric ≥ 14/20.
...
Pattern: Pulse + Magi set CVR target → Funnel structures + Growth optimizes acquisition → Experiment designs first variant → Voice + Saga supply proof. `lure` enforces CVR target met, messaging hierarchy clean, variant queued, analytics live.
...
Pattern: Growth audits + applies → Bolt enforces perf → Funnel + Prose author heading tree → Polyglot localizes. `lure` enforces Technical SEO checklist 100%, Lighthouse SEO ≥ 95, GEO Rubric ≥ 15/20.
...
Pattern: Funnel designs structure → Prose writes headings as claims → Echo walks through with persona → Canvas visualizes journey → `lure` scores IA Rubric ≥ 15/20 at STRUCTURE gate.

### Axis Coordination

`lure` runs a coherence audit at each major gate:
