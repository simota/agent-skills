...
(venture preset). For a corporate strategy review, BMC pairs with `magi scenario`/`swot`.
...
- **Handoffs:** value-proposition zoom-in → `spark` `value-proposition-canvas.md`;
  ...
  instrumentation → `pulse`; financial modeling of Revenue/Cost → `magi forecast`
