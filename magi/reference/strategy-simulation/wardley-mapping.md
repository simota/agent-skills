...
- **magi `wardley`**: Simon Wardley value-chain + evolution mapping (this document).
- **magi `porter` (elsewhere)**: Industry structure analysis. Porter is static; Wardley is evolutionary.
- **magi `jtbd` (elsewhere)**: Customer-job reframing. Wardley starts with user needs and traces the chain backwards.
- **magi `blue-ocean` (elsewhere)**: New market creation. Wardley reveals *where* on the map to create new space.
