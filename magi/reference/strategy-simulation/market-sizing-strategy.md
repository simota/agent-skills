# Market Sizing for Strategic Context — Magi

Purpose: Use this file when Magi needs to incorporate market size data into strategic simulation, market entry evaluation, portfolio allocation, or growth planning.
...
| Strategic interpretation | **Magi** | Using market size for decisions, simulations, forecasts |

Magi does not estimate market size from scratch. Magi consumes market sizing data from Compete (via `COMPETE_TO_MAGI` handoff) or from provided context, and applies it to strategic frameworks.

### When Magi Needs Market Sizing
...
Integrate market sizing into Magi's 3-scenario model:
...
- Feed attractiveness score into Magi scenario simulation
...
If this handoff data is not available, Magi should request it via `MAGI_REQUEST_COMPETE` or note the gap explicitly in assumptions.
...
### Strategic Implication for Magi Scenarios
