...
Reference for Grove[llm]'s `sharding` recipe. Split large CLAUDE.md / reference docs via `@import` while preserving prompt cache prefixes and avoiding circular references.

> Complements Hone (density audit). Hone identifies *what* to extract; Grove[llm] decides *where to put it* and *in what order*.
