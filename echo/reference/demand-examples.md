...
> Card UIs might look pretty, but reading them with a screen reader is hell. When information is spatially arranged, the reading order makes no sense. The current list view is much easier to use. If you're redesigning, please at least keep a table view option. Echo[demand]se.
...
> Echo[demand]se reduce API response time to under 200ms. Fixing the current N+1 queries should improve it.
