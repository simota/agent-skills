...
**Purpose:** Templates and authoring rules for the paste-ready LLM instruction prompts Echo[demand] pairs with every demand and every report.

Echo[demand] emits two granularities. Both are mandatory output — not optional.
...
| `REFINE` | Iterate on existing demand — add detail, narrow scope, resolve contradictions | Echo[demand] (self), Field |
...
```text
You are receiving a synthetic user demand generated by Echo[demand] (user advocate).
...
```text
You are receiving a User Demand Report from Echo[demand] covering [scope].
