...
- **`reversibility`** — `classification` (`HIGH` single flag / single-commit revert, no data migration · `MEDIUM` coordinated rollback + migration window · `LOW` effectively a new project), plus `revert_procedure`, `revert_time_estimate`, and `revert_blast_radius`.
