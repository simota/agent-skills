...
- **Schema `tenant mode=quota`**: per-tenant rate limits, fair-share scheduling, soft/hard quota policy, burst budgets, overage-billing handoff.
- **Schema `tenant mode=isolation` / `rls` / `routing` / `scale` (elsewhere)**: structural tenant isolation, RLS, routing, and shard-level capacity. Quota is the *runtime* fairness layer on top of `scale`.
