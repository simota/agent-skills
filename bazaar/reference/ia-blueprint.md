...
Information Architecture, technical SEO, content SEO, and AI-search GEO are interleaved disciplines. `bazaar` treats them as one continuous axis: structure → readability → discoverability → citation-readiness.
...
Composite gate enforced by `bazaar` at Optimize stage exit:

| Sub-gate | Threshold | Owner |
|---------|-----------|-------|
| IA rubric | ≥ 15/20 | `bazaar` + Funnel |
