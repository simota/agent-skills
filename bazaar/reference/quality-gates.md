...
Per-stage exit criteria for the `bazaar` LP studio chain. A stage that fails its gate either repairs in place (≤1 retry) or escalates back to the user.
...
Gate evaluation is owned by `bazaar` (not the delegate). `bazaar` reads the delegate's output, checks against the gate's measurable criteria, and chooses outcome.

### Oscillation Guard

If the same criterion flips PASS↔FAIL across 2 retries (same axis or cross-axis), `bazaar` skips the standard 1-retry rule and escalates immediately to Magi for trade-off arbitration. Examples:
...
|-------|-----------|-------|-----------|
| One-promise discipline | Every stage | `bazaar` | Brief carries one primary value prop. Two-promise drift = stop and ask. |
| Token discipline | Build, Optimize, Verify | `bazaar` | No hardcoded user-facing values in production code |
| Brand alignment | Design, Build, Optimize | `bazaar` | Production rendering matches Vision direction.md and Brand System record |
| Persona alignment | Structure, Design, Verify | `bazaar` | Copy, design, and tested flow reference the same primary persona |
| Performance budget | Build, Optimize | `bazaar` + Bolt | LCP ≤ 2.5s, INP ≤ 200ms, CLS ≤ 0.1 throughout |
| A11y baseline | Design, Build, Verify | `bazaar` + Palette | WCAG 2.2 AA never regresses |
...
|------|--------|-----------|-------|--------|
| **Design** | Visual Hierarchy Rubric (9 criteria × 3 points; Hero-Contract Legibility added) | ≥ 20/27 | Vision + Muse + Palette + `bazaar` (or Atelier — rubric still enforced) | `craft-standards.md` § Design Discipline |
...
1. `bazaar` flags the failing criterion with score and gap.
...
After each stage, `bazaar` emits a stage report in this shape and persists to `.agents/bazaar/{project}.json`:
