...
How `bazaar` brokers work between stages and how it dispatches each delegate. Every delegate call carries a `LURE_STAGE_BUNDLE`. Every stage exit produces a `STAGE_REPORT`. There is no free-form delegation in `bazaar`.
...
`.agents/bazaar/{project}.json` is updated by `bazaar` only. Delegates return values; they do not write state. Write protocol:

1. Read current state.
2. Compute merged state (append-only on `decisions_log`, replace-or-insert on `stages.<STAGE>`).
3. Write to `.agents/bazaar/{project}.json.tmp`.
4. Atomic rename `.tmp` → `.agents/bazaar/{project}.json`.
...
Schema in `_common/HANDOFF.md`. `bazaar` reads `Constraints`, `Scope`, and `Acceptance_Criteria`, selects Recipe, and emits its first `LURE_STAGE_BUNDLE`.
...
`bazaar` either re-enters at Optimize or Verify (scoped repair) or escalates to user.
...
State per project lives at `.agents/bazaar/{project}.json`:
