...
| **hooks** (Hone's domain) | Shell/prompt/http/agent commands fired at lifecycle events (PreToolUse, PostToolUse, Stop, SessionStart, …) | Per-event reactions *inside* the loop — gates, logging, context injection, completion enforcement |
...
## How this informs Hone hook design
...
Hone owns the **hook** primitive only. The loop's cadence/contract/recovery belongs to **Orbit**; multi-engine orchestration to **Nexus**. When a request is "build me the whole loop," route the loop design to Orbit/Nexus and supply the hooks they need.
