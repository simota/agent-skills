...
- **`exit 0` always** — telemetry is monitoring, not enforcement. Fail-open per Hone Core Contract.
...
1. Backup `settings.json` (Hone Core Contract): `cp ~/.claude/settings.json ~/.claude/settings.json.bak`
...
- `chain[malware-scan]` — when telemetry surfaces an unknown skill firing (potential supply-chain malware), route to Chain for an IoC scan.
