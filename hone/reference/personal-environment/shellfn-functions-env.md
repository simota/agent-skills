...
Purpose: Read this when organizing shell functions and aliases, managing `PATH`, wiring per-project tooling with `direnv` / `mise` / `asdf` / `nvm`, enforcing XDG directories, and lazy-loading heavy completions. Hone owns **your** personal shell function layout; publishable CLI tools and team tooling manifests belong elsewhere.

## Scope Boundary

- **Hone `shellfn`**: personal shell functions, aliases, env-var strategy, PATH hygiene, `direnv` / `mise` / `asdf` / `nvm` wiring, XDG migration, lazy-loading completions.
- **Builder (elsewhere)**: publishable CLI tools — when a function grows flags / help text / tests / release needs, hand it off.
- **Hone (elsewhere)**: Claude Code hooks that invoke shell commands — Hone provides the env, Hone wires the trigger.
...
- Function outgrowing aliases / needs flags, help, tests, release → `Builder`.
- Claude Code hooks invoking these functions → `Hone`.
- `.tool-versions` / `mise.toml` as team contract → `Gear` + `Grove`.
- PATH/env audit reveals secrets or risky eval in startup → `Sentinel` + Builder `audit`.
- Reproducing env on a new machine → Hone dotfile-manager recipe (stow / chezmoi).
