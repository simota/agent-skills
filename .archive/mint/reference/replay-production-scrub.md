...
- **Canon[regulatory] (elsewhere)**: regulatory approval for production-capture programs (GDPR Art. 32, HIPAA safe-harbor).
...
If the question is "can I share this traffic capture with CI?" → `replay`. If it is "are we allowed to capture it in the first place?" → Cloak + Canon[regulatory].
...
| Golden regression set (long-lived) | 1y, quarterly re-scrub | requires Canon[regulatory] approval + re-validation |
...
**To Cloak / Canon[regulatory]:** periodic audit evidence — scrub ruleset version, bundle inventory, expiry compliance — for DPIA and audit trail.
