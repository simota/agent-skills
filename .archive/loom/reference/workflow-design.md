...
Every workflow Loom designs MUST declare one topology. Unstructured flat peer networks ("Bag of Agents") amplify errors and never ship.
...
In this ecosystem the runtime hub is `Nexus`. A project workflow Loom designs is registered with Nexus, which acts as the hub at execution time. Loom declares the topology; Nexus runs it.
...
- **Loop needed** (the workflow runs itself repeatedly toward a goal until a condition): Loom specs the loop and hands it to `Orbit`. Loom does not build the runner.
- **Code-level state machine / saga** (states, transitions, compensation in the application): that is `Weave`'s domain. Loom does not design state machines; it may reference one a workflow depends on.
- **Runtime routing** of the workflow per request: `Nexus`. Loom designs the persistent definition; Nexus executes.
