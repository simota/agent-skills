...
Delegate deep code reading to `Lens` and reuse `Sigil`'s inventory; Loom synthesizes, it does not re-scan line by line.
...
**Greenfield repos** (no `.claude/` layer, no prior Sigil runs): infer stack and conventions directly from manifests, CI config, scripts, and commit history; Loom owns this existing-layer scan itself. Sigil's inventory is consulted only when a prior run produced one — never block on its absence.
...
For each recurring task or pain point from SURVEY, pick the cheapest mechanism that solves it. Use `_common/MECHANISM_SELECTION.md` for the hook / scoped rule / subagent / skill choice; `recipe` and `workflow` are **Loom-local mechanisms** defined here and in `recipe-design.md` / `workflow-design.md` (the `_common` file does not cover them). Combined summary:
...
| A **repeatable multi-step task** chaining skills/agents | Recipe (subcommand flow) | designed by Loom, registered via `Nexus` |
| A **multi-agent outcome** needing coordination | Workflow / chain | designed by Loom, run via `Nexus` (loop → `Orbit`) |
