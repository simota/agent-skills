...
- [ ] **L2** Each chosen mechanism is the cheapest that solves the task (no skill where a hook/rule suffices; no workflow where a recipe suffices). Cross-checked against `_common/MECHANISM_SELECTION.md` (hook/rule/subagent/skill) and `recipe-design.md` / `workflow-design.md` (recipe/workflow — Loom-local mechanisms).
...
- [ ] **L11** No authored skill bodies, executed chains, or installed files are present in Loom's output — design and handoffs only.
...
| Workflow has 8 phases | L8 | Loom redesign → Nexus re-register |
