...
**Read when:** You are in CHANNEL (default `channel` recipe or the `critique` recipe).
...
5. **Stop before the verdict.** Summon advises; the decision goes to Magi or the user.
...
**Next:** decision → Magi/user · more lenses → conclave · write-up → Scribe
```
