...
**Read when:** You are running the `conclave` recipe (multi-figure panel).

## Principle

A conclave's value is **divergence**, not agreement. If three figures would genuinely clash, the clash *is* the deliverable. Summon never resolves the clash into a verdict — that is Magi's job. Hand off when the user actually needs to decide.
...
5. **Offer the handoff.** If the user must choose, route the panel to Magi (`SUMMON_TO_MAGI`) for arbitration, or back to the user. Summon advises.
...
**Next:** decide → Magi/user · deepen one lens → channel · write-up → Scribe
```
...
- **Verdict creep** — declaring a winner. Surface the tension; let Magi/user decide.
