...
| W11b | **No-padding calibration is mandatory on every doc-authoring spawn** | **Opus 5's files-to-disk output runs longer than prior models' — a second, independent channel from conversational verbosity, and lowering `effort` does not shrink it.** A doc spawn that omits an explicit length instruction will over-produce, and the padding lands in filler sections, restated summaries, and boilerplate — exactly the material W10/W11 exist to prevent. Every Scribe/Scribe[unified]/Quill/Tome/Morph spawn therefore carries, alongside its P2 envelope: *"Match the length of written documents to what the task needs: cover the substance, but do not pad with filler sections, redundant summaries, or boilerplate."* For a multi-file package, state the envelope **per file**, not for the set — a set-level budget silently reallocates into whichever file the model writes first. |
...
- **Any chain step authoring a doc** (Scribe/Scribe[unified]/Quill/Tome/Morph) — the spawn prompt carries the W1 reader contract, the W4–W5 grounding duty, and the **W11b no-padding calibration** in its output envelope; the hub runs W12 at VERIFY.
- **Multi-file document recipes** (`chronicle`'s per-lens deep-dive files, `cartograph`'s map set, `package` presets) are the highest-risk shape for W11b: a per-file envelope is required, and the split must be justified as a **reader-navigation choice**, never as a context workaround — Opus 5 serves a 1M-token window by default, so "the content wouldn't fit" is no longer a reason to shard a deliverable.
