...
| W11b | **Bound written artifacts by reader needs** | Give each authored artifact its reader, required substance, evidence and appropriate size ceiling. For multi-file deliverables, identify each file's purpose instead of filling a package-wide quota. Reuse the shared P2 envelope; do not duplicate model-specific no-padding slogans in every spawn. Completeness outranks an arbitrary length target. |
