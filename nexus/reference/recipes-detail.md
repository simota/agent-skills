...
| **Improve** (existing code) | `refactor` · `optimize` · `kaizen` · `anneal` · `restyle` | refactor = apply a *known* internal restructure (known scope) · optimize = perf-only (a number) on *correct* code — a slowdown caused by a defect (wrong output, memory leak, regression from a change) is `bug`/`+Trail`, not `optimize` · kaizen = multi-axis polish of *one feature* vs a known target · **anneal = *discover* undiagnosed design weaknesses across a scope → brush up the prioritized slate, behavior-preserving** · **restyle = improve the *visible* design (UI · visual · interaction) of an existing surface, direction+rubric-driven, behavior-preserving**. **`improve`/`polish`/`enhance` is overloaded across all five → REDIRECT** (`audit the design` / `brush up the codebase` / `harden the architecture` → `anneal`; `improve a feature vs a target` → `kaizen`; `known single restructure` → `refactor`; `iterate to a quality bar` → `converge` in the **Loop** family; `redesign` / `UI refresh` / `look and feel` → `restyle`). **`improve the design` alone is overloaded on the code-vs-visual axis → REDIRECT** (architecture/code design → `anneal`; UI/visual/look-and-feel → `restyle`). |
...
| **Quality-Max** (expensive, confirm) | `acceptance` · `growth-acceptance` · `summit` · `podium` · `wish` · `runway` · `hallmark` · `rebrand` · `marquee` | acceptance = proof-carrying merge (G1-10) · growth-acceptance = post-launch lifecycle (G11-15) · summit = pre-merge quality tournament · podium = content/slide quality · **wish = once-in-a-lifetime one-shot ceiling: scarcity-gated, deliverable-agnostic wrapper — crystallization dialogue + tournament + adversarial gauntlet + ceiling convergence (ACCEPT = all rubric dims = 3) + One-Shot Gate. Axis vs siblings: summit = *engine triangulation for strategic code*; converge (Loop family) = *standard bar, all dims ≥ 2, revisions expected*; wish = *the single most important ask, no second delivery*.** **Design/brand wing:** runway = flagship *in-product UI surface* design tournament (3 directions → ceiling, all dims = 3) · hallmark = *create* the brand identity (proof-carrying Brand Book) · rebrand = *propagate* a settled brand across all surfaces (completeness-proven, RESIDUE-GATE) · marquee = wish-grade one-shot *acquisition LP* (machine oracles Lighthouse/CWV/WCAG; **no Scarcity Gate — intentional vs wish**). Axes: `runway` vs `marquee` = in-product surface vs conversion-driven LP; `hallmark` → `rebrand` = create→propagate pair (a discover→build pair living in this family for its cost/gates). |
...
**Once-in-a-lifetime request** — scarcity-gated **one-shot quality-ceiling** delivery, deliverable-agnostic (code / document / design / content / plan). Four identity elements no sibling carries together: **Scarcity Gate** (usage journaled in `.agents/nexus.md`; routine invocations challenged — anti-inflation is contract-level), **Wish Crystallization** (contract-level dialogue excavating the true wish + *disappointment criteria*), **Ceiling ACCEPT** (all rubric dims **= 3**, raised from converge's ≥ 2), and **One-Shot Gate** (a fresh-context independent verifier — no loop history — answers "would a fresh redo be materially better?" with cited evidence before the single delivery; may grant exactly one bonus cycle, second verdict advisory-only). 15-40 agents × ≤4 cycles, 6-18× cost. **Always confirm** (the Scarcity Gate is the confirmation; intentional parity with summit's unconditional gate). Distinct from `summit` (multi-engine tournament for strategic *code*), `converge` (standard bar, revisions expected), `apex` (build lifecycle), `podium` (content pipeline).

**Chain template:**
`Phase 0 Scarcity Gate ★confirm-before-launch (Wish Ledger + outcome backfill + preliminary cost + 2-of-3 routine-task challenge) → 1 Crystallize ★contract-level dialogue (Wish Contract + Ceiling Rubric: 3-5 dims, task-specific score-3 anchors, reachability-checked; disappointment criteria as score-0 triggers) → 2 Tournament (3 diverse-angle candidates, outline-level for S/M scope ‖ → judge panel = Evaluator roster → winner + salvage list) → 3 Gauntlet-Converge ≤3 cycles (independent Evaluators per dim ‖ staged refutation panel per _common/ADVERSARIAL_REFUTATION.md once all dims ≥ 2; dims = 3 → candidate ACCEPT → panel ratifies) → 4 One-Shot Gate (fresh-context producer≠verifier redo-verdict with cited evidence; ≤1 bonus cycle, re-gate advisory-only) → 5 Deliver (Fulfillment Report + Wish Ledger append)`
