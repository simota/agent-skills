...
| **Comprehend** (reverse-engineer existing code → understanding artifact, no code) | `cartograph` · `chronicle` | output = a grounded documentation artifact of *what exists*, not insight/evolution and not a build. **Within-family axis: space vs time** — `cartograph` = multi-repo structure, how it works *today* (code citations); `chronicle` = commit history, how it *got here* & what it believes (commit SHAs). Axis vs the **Reason** family: document-what-exists (backward) vs reason/evolve (forward). Full prose + the route-out list → §§ `cartograph` · `chronicle` below. |
