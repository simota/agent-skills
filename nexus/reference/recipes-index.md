...
| Optimize | `optimize` | | Performance-only improvement on *correct* code — measure-first, prove-with-a-number. Defect-caused slowdown → `bug` | `[locate layer] → Bolt/Tuner/Gear/Scaffold[measure→validity→target→optimize] → Radar[verify-speedup, independent] → Guardian`| `reference/optimize-recipe.md` (deep contract), `reference/routing-matrix.md` (task-type row) |
