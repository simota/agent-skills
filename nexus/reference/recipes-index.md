...
| Eureka | `eureka` |  | **Novelty-proven invention** — name the contradiction current solutions accept as given, sweep prior art (inverted polarity: research to *avoid*) + failure archaeology, diverge across six structurally-different generators, run the **two-test novelty gate** — `COLLISION` (killed with its citation) and `OBVIOUS` (killed by a **blind ordinary-move panel** that pre-commits its proposals before any candidate exists), survive the **tetrad gate** (novelty · value · feasibility · defensibility — a **rights side-check** runs alongside but is never an axis: it gates the autonomous path, not the invention verdict), and prove the mechanism with a **falsification-first** spike before disclosing it. **Always confirm.** A `BLOCK (prior-art saturation)` run still ships its dossier. **`ship=true`** (opt-in, never inferred) continues end-to-end: Ship Gate (6 preconditions + a second confirm and a separate build envelope) → collapsed `spec` → `apex` under a **Novelty Invariant** → novelty regression check → ship; 32-61 agents invention-only × ≤3 cycles (+1 re-ideation), 9-24× cost; 43-91 combined, 13-33× cost. | See `reference/recipes-detail.md` | `reference/eureka-recipe.md`, `reference/research-grounding.md` |
| Wish | `wish` |  | **Once-in-a-lifetime request** — scarcity-gated one-shot quality-ceiling delivery: crystallize the true wish → **anchor the ceiling to sourced best-in-class exemplars** → **cross-engine blind tournament** → adversarial gauntlet + ceiling convergence on **calibrated** evaluators (all dims = 3) → reception simulation → **One-Shot Gate + Comparative Gate**. Budget-enveloped. **Domain preset:** `domain=lp` (alias `marquee`) fixes a 5-dim LP rubric with machine oracles and drops the Scarcity Gate. 27-102 agents × ≤5 cycles (+1 bonus), 10-28× cost. **Always confirm.** | See `reference/recipes-detail.md` | `reference/wish-recipe.md`, `reference/research-grounding.md`; domain=lp blueprint → `reference/marquee-recipe.md` |
