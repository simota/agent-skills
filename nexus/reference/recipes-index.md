...
| `eureka`, `invent`, `invention`, `breakthrough`, `genuinely new`, `nobody has done this`, `patentable`, `novel mechanism`, `prior art`; **+ "and build it" / "invent and ship" / "発明して実装まで" → `eureka ship=true`** | `eureka` (novelty-**proven** invention — a mechanism already proven elsewhere → `graft`; picking among candidates in hand → `killer`; feature ideation inside the known solution space → `spark`) |
