...
## Execution Model (SKILL.md excerpt)
...
Per-CLI spawn API: Claude Code `Agent` (L1 fg / L2 background / L3 Rally); Codex `spawn_agent`→`wait_agent` (prereq `multi_agent=true` + `max_depth>=2`); agy `/agent` in TUI or `agy -p --dangerously-skip-permissions` headless (prereq TUI main session or OS-level isolation). Full prereqs, runtime notes, silent-failure mitigations, and the verified headless template → `reference/execution-layers.md`; cross-CLI mapping → `_common/CLI_COMPATIBILITY.md`.
...
## Execution Model — Condensed (SKILL.md excerpt)

**Orchestrator detection** — detect which CLI drives *this hub session* once, before the first spawn (`Agent` → Claude Code; `spawn_agent` → Codex CLI; `/agent` in a TUI main session → agy), then bind the spawn API, authoring protocol, and model map. Detection table + Codex prereqs (C1) → `reference/hub-authoring.md`.
...
**Execution layers** — Claude Code `Agent` (L1 fg / L2 background / L3 Rally) · Codex `spawn_agent`→`wait_agent` (prereq `multi_agent=true` + `max_depth>=2`) · agy `/agent` in TUI or `agy -p --dangerously-skip-permissions` headless (prereq TUI main session or OS isolation). Prereqs, runtime notes, silent-failure mitigations, headless template → `reference/execution-layers.md`; key rules → `reference/hub-authoring.md`.

> **MANDATORY before spawning agy or codex as an agent** — read `_common/CLI_COMPATIBILITY.md §9.2` (agy headless MUST allocate a real pty via `python3 pty.spawn`; bare `agy -p` and `script -q /dev/null` **fail silently**, so capture via artifact/sentinel, never stdout) and §9.3 (codex `-o <abs path>` artifact is authoritative). These are silent-output regressions, not edge cases.

**Model selection** — names are hub-engine-specific, role → tier mapping is stable. Table → `reference/hub-authoring.md` § Model Selection; cross-CLI → `_common/CLI_COMPATIBILITY.md §4`.
...
## Operational Notes for Spawns (SKILL.md excerpt)
