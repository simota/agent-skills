...
## Chain template
`FRAME (Plea +Field?/Cast? + Socratic dialogue) → ✓confirm-problem → EXPAND (Riff ‖ Flux +Compete?) → ✓steer → CHALLENGE (Magi + Void + Ripple +Omen?) → ✓pick + convergence-check → SHAPE (Spark +Rank?) → ✓edit → SPECIFY (Accord +Scribe?/Gateway?/Schema? + L3 ACs mandatory, Attest? +Echo?) → ✓iterate → LOCK (✓sign-off → write docs/specs/<slug>.md + Open Questions section → ✓build-path: orbit loop ‖ apex (fallbacks: feature|acceptance|essential|killer) → recommend chosen handoff) [NO CODE]`
