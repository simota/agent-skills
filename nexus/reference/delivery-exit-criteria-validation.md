...
Purpose: Read this file when Nexus[deliver] validates a phase transition, applies pass thresholds, or uses simplified `S/M` validation and override rules.
...
Defines automated validation of phase exit criteria before Nexus[deliver] approves phase transitions.

---

## Overview

Before any phase transition, Nexus[deliver] runs an automated validation chain to verify exit criteria are met. This prevents premature phase advancement and catches quality gaps early.
...
4. Log the conditional pass with gaps in NEXUS_DELIVERY_STATE
