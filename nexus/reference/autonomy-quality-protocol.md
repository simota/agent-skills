...
| # | Rule | Discipline |
|---|------|-----------|
| Q12a | **Spend effort where it changes the verdict, not uniformly** | Inherit the authorized runtime and use supported effort controls only when task evidence warrants a change within budget. Do not assume that one generation makes low-effort review reliable, or steer private thinking with ritual phrases. Keep the same rubric, ACs and independent verification at every effort setting. Set visible output limits separately. Verify provider-specific cache and effort behavior through `_common/CLI_COMPATIBILITY.md`; measure successful-task cost including retries before changing a policy. |
