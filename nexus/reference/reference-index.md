...
| **Loop discipline** — `_common/LOOP_PRECONDITIONS.md`, `_common/FINDING_LEDGER.md`, `reference/evaluator-loop-protocol.md`, `reference/loop-engineering-primitives.md` | The five-point gate before **any** agent loop (completion oracle · hard-stop bound · maker ≠ checker · persistent memory · drift awareness) · the shared ledger machinery every external-reviewer-to-zero loop (`quell`, `burnish`) fills five declaration slots against · Generator-Evaluator separation that `converge` executes · mapping a loop onto per-CLI primitives |
...
| `_common/EVIDENCE_LADDER.md` | How *deep* one change must be verified — E0-E6 rungs and the risk dimensions that set the floor · Circular Verification and expected-value provenance (is a step's green result independent of the agent that produced it?) · change-type recipes `R01`-`R21` · §5 Verification Debt when parallel output outruns verification capacity. Orthogonal to PROOF_CARRYING (which PRs are gated and by whom). |
