...
*Range derivation (keep reconciled when a phase figure changes):* floor = narrow scope, `apply=false`, no re-experiment → 1+2+4+1+12+3+0+1 = **24**; ceiling = whole-codebase scope, `apply=true`, one re-experiment round (+5) → 1+4+7+1+27+5+4+1+5 = **55**. The stated range is the sum of the per-phase figures, not an independent estimate. **Confirm-before-launch always** (intentional parity with `wish`/`summit`/`eureka`) **with a declared budget envelope**; `apply=true` confirms a **second** time at Phase 5.
