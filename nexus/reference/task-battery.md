...
Items 1-30 are Candidate A's original battery (routing-level verification for the Recipe Families + ambiguous/edge inputs). Items 31-35 are Phase-2 salvage fold-ins (S9): 2 additional ambiguous-anchor stress tests + 3 additional out-of-coverage items (beyond A's patent-filing / travel-booking) to broaden the dim-3 (Coverage closure) proof beyond two data points. Items 36-37 cover `FIGURE_CHANNELING` (named-figure routing); items 38-39 cover the `eureka` invention/build split — the pair exists to prove `ship=true` is **triggered only by explicit build intent** and never inferred from an invention ask. C's ecosystem-l10n item was explicitly discarded (S9: "partial overlap" with `polyglot`, not a clean out-of-coverage case).
...
| 36 | "how would Feynman approach debugging this?" | Thinking (named figure) | `FIGURE_CHANNELING` (Summon) — a **real named individual** is the trigger; not `flux` (no person named) and not `magi` (no verdict asked for) |
| 37 | "get me a panel of Buffett and Munger on this pricing decision, then tell me what to do" | Thinking → Decision | `FIGURE_CHANNELING` chain `decide`: `Summon[conclave] → Magi → Builder` — Summon contrasts the named experts, Magi issues the verdict Summon never does |
