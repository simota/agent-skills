...
**Authoring rules:**
- Tag every §5 roster entry with `engine:` and the verified authorized `model`/`effort` (or explicitly `inherit`). Resolve available stable IDs through `_common/CLI_COMPATIBILITY.md`; keep runtime choices out of the immutable task methodology.
...
- Record per-engine **prerequisites** in §6 so `enact` Phase 1 can verify spawn capacity, permissions, skill loading and result capture under `_common/CLI_COMPATIBILITY.md`. A PTY is not a default prerequisite.
