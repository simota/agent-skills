...
| XQUIK | Xquik | Xquik[read/monitor/integrate/act] | +Relay (webhook delivery), +Stream (pipeline), +Gateway/Builder (product integration), +Voice/Compete/Pulse (analysis), +Prose/Sentinel (approved action input). Public reads use least authority; private reads and every account action require the documented account connection; side effects, new payments, and top-ups require explicit approval |
