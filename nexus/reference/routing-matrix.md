...
| MACOS_AUTOMATION | Hearth | Hearth[automate] | +Tempo (schedule as cron/launchd), +Anvil (graduate to packaged CLI), +Latch (wire as Claude Code hook), +Vector (web step before native step), +Sentinel (security screen generated do-shell-script/subprocess), +Scout (diagnose broken automation). macOS-only; dictionary-first, TCC-aware, destructive ops gated behind dry-run |
...
| CREATIVE | Vision → Sketch → Artisan | Sketch[generate], Ink[icon] | +Sketch[edit] (image edit), +Sketch[prompt] (prompt opt), +Ink[illustration] (illust), +Growth (marketing). **Image-generation engine default**: when actual image assets are the deliverable, the primary engine is Codex built-in `image_gen` (ChatGPT subscription, no API billing) — instruct Sketch to emit Codex operating guidance per `sketch/reference/codex-image-gen.md`; use the Gemini API code path when the deliverable is reproducible generation code (seeds/batch/metadata) or fine parameter control is required |
...
| MACOS_NATIVE | Native[macos] → Radar → Vitrine → Launch | Native[macos], Native[macdist] | +Forge (prototype validation), +Vision (Mac design direction — `vision/reference/apple-design-trends.md`), +Muse (tokens), +Hearth[automate] (automation-readiness / scripting surface of the shipped app), +Cloak (privacy + TCC purpose strings), +Crypt (Keychain / code-signing key handling), +Gear (CI signing + notarization wiring). Pure macOS native — Catalyst-vs-native is an explicit decision (`native/reference/catalyst-decision.md`); iOS/Android → `MOBILE_NATIVE`; automating an *existing* Mac app (AppleScript/JXA) → Hearth `automate`, not Native |
| IOS_UI_TEST | Snap → Gear → Launch | Snap[xcuitest], Snap[identifier], Snap[screenshot], Snap[appstore], Snap[page-object], Snap[ci], Snap[farm], Snap[xcresult] | +Gear (CI wiring for xcodebuild test + xcresulttool), +Launch (App Store screenshot release step). Pure XCUITest scope — Appium/Detox/Maestro cross-platform E2E → Voyager |
...
| SUPPLY_CHAIN_AUDIT | Chain | Chain[manifest], Chain[scan], Chain[intake], Chain[pin] | +Sentinel (app-side SAST contrast), +Gauge (SKILL.md format audit), +Latch (hook design), +Gear (CI/CD config). Don't confuse with Cull (active malware infection) or SECURITY (application vulnerabilities) |
| FIGURE_CHANNELING | Summon | Summon[channel], Summon[conclave], Summon[critique], Summon[roster] | +Magi (the reading feeds arbitration when a verdict is actually needed — `SUMMON_TO_MAGI`), +Flux (reframe first, then channel a thinker known for that frame), +Riff (expert mental models as ideation seeds), +Sage (founder problem needing a specific named investor/founder), +Scribe/Quill (write the reading up). **Advisory lenses only, never a verdict** — decisions → Magi, open brainstorming → Riff, synthetic user personas → Cast, fixed founder-mentor archetype → Sage. Every claim carries an ATTESTED/INFERRED/SPECULATIVE tag; fabricated quotes are stripped; deceptive uses (impersonation, fabricated endorsement) are refused |
| MALWARE_RESPONSE | Cull → Triage → Crypt | Cull[scan], Cull[eradicate], Cull[rotate], Triage[respond], Crypt[rotation] | +Vigil (Sigma/YARA rule authoring), +Sentinel (static rescan), +Chain (re-audit `.claude/` tree), +Trail (git history of infection vector). Always run Cull first — wrong credential rotation order triggers `rm -rf ~/` retaliation payload |
