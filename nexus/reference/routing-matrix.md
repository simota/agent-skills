...
| DECISION | Magi | Magi[decide] | +Scribe[unified: vision] (biz-tech), +Flux (reframe), +Riff (explore before deciding) |
...
| REQUIREMENTS | Scribe[unified] → Scribe → Sherpa | Scribe[unified: vision], Scribe[prd], Sherpa[epic], Plea[request], Plea[need], Saga[story], Saga[scenario] | +Canvas (diagram), +Magi[decide] (decision), +Saga (narrative), +Cast[generate] (persona) |
...
**Add-ons**: `+Crypt` (cryptographic design fix), `+Shift` (dependency CVE → upgrade), `+Cloak`/`+Canon[regulatory]` (privacy/compliance dimension), `+Sentinel` re-scan after fix, `+Latch` (recurrence-prevention hook, distinct from Vigil detection — see note above).
