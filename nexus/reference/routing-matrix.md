...
| MARKETING | Cast → Compete → Pulse → Saga → Growth → Funnel → Experiment | Cast[generate] (persona), Compete[matrix]/[swot] (competition), Pulse[kpi] (metrics first), Saga[story] (narrative), Growth[seo]/[cro]/[geo] (channels), Funnel[landing] (LP), Experiment[ab] (validation), Voice[nps] (VoC), Echo (message-reception check), Magi (positioning verdict) | +Sherpa (full chain, mandatory for ≥7 steps), +Field (qualitative persona derivation), +Helm (scenario sim), +Vision (product brand), +Compete[brand] (personal brand only), +Prose (copy), +Muse (brand tokens), +Trace (churn/session analysis), +Bond (churn ops), +Launch (GTM execution and reporting), +Echo[demand] (synthetic user voice when no real customers), +Magi[advisor] (advisor pressure-test), +Flux (reframe), +Matrix (multi-segment). **Boundary vs STRATEGY**: MARKETING/* delivers strategy/messaging/plan; STRATEGY/{seo,compete,retention,metrics,ab-test} delivers code/instrumentation for a single tactic. |
...
| MACOS_AUTOMATION | Anvil | Anvil[automate] | +Tempo (schedule as cron/launchd), +Hone (wire as Claude Code hook), +Vector (web step before native step), +Sentinel (security screen generated do-shell-script/subprocess), +Scout (diagnose broken automation). macOS-only; dictionary-first, TCC-aware, destructive ops gated behind dry-run |
...
| DEV_EXPERIENCE | Anvil → Gear → Hone | Anvil[env], Gear[deps], Hone[hook] | +Sigil (project skills), +Hone[audit] (CLI audit) |
...
| BRANDING | Compete → Quill | Compete[brand], Compete[brand], Compete[brand], Compete[brand], Compete[brand] | +Growth (SEO/SMO), +Canvas (viz), +Prose (content), +Launch[pr-flow] (portfolio evidence) |
...
| ARTICLE | Tome → Growth | Tome[note], Tome[zenn], Tome[qiita], Tome[devto], Tome[article-series] | +Prose (microcopy polish), +Stage (slide version), +Saga (narrative reshape), +Canvas (article diagrams), +Morph (PDF/Word export). Trigger: "tech blog", "note/Zenn/Qiita/dev.to", "article series", "article" |
...
| FIGURE_CHANNELING | Magi | Magi[advisor], Magi[advisor], Magi[advisor], Magi[advisor] | +Flux (reframe first, then apply a thinker known for that frame), +Riff (expert mental models as ideation seeds), +Scribe/Quill (write the reading up). **Advisory lenses are not a verdict** — choose Magi[decide] explicitly when arbitration is requested; open brainstorming → Riff; synthetic user personas → Cast. Every claim carries an ATTESTED/INFERRED/SPECULATIVE tag; fabricated quotes are stripped; deceptive uses are refused |
