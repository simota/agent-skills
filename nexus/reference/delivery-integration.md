...
Purpose: Read this file when Nexus[deliver] receives `## NEXUS_COMPLETE_[STATUS]`, validates resulting artifacts, maps status into Anti-Stall, or updates `NEXUS_DELIVERY_STATE`.
...
- `NEXUS_DELIVERY_STATE` update rules

Defines how Nexus[deliver] validates and responds to Nexus chain completion results.
...
| Status | Meaning | Nexus[deliver] Response |
|--------|---------|---------------|
| **FULL** | All steps completed, all acceptance criteria met | Update NEXUS_DELIVERY_STATE → next Epic |
...
Nexus includes a `recovery_attempted` field in `NEXUS_COMPLETE_[STATUS]` to indicate whether internal recovery was already executed before reporting to Nexus[deliver]. This prevents duplicate retry efforts and wasted Anti-Stall budget.
...
NEXUS_COMPLETE_[STATUS] received
  ...
  ├─ 4. Update NEXUS_DELIVERY_STATE with results
...
|-------------|---------------------|-----------------|-----------|
| PARTIAL | `false` | **L1** (1.1 Retry) | Nexus didn't retry; Nexus[deliver] retries with context |
...
**Rule**: When `recovery_attempted: true`, Nexus[deliver] skips the recovery level that Nexus already exhausted and enters Anti-Stall at the next effective level. This prevents duplicate retries and conserves Anti-Stall budget.
...
4. **Persist**: Write updated state to `.agents/nexus-delivery-state.md`
