...
**Default baseline: Claude + Codex (dual-engine).** agy is an optional third axis when AVAILABLE and authorized; use its verified capabilities rather than a frozen model/effort mandate. Dual-engine is normal, not degraded. Preserve `_common/MULTI_ENGINE_RECIPE.md` availability and provenance rules.
...
**Engine routing principle:** the matrix below expresses this recipe's deployment defaults, not universal vendor rankings. Verify tool access, context needs, model availability and authorized cost under `_common/CLI_COMPATIBILITY.md`; preserve specialist ownership and independent verification when adapting an engine.
...
| Creative divergence / alternative exploration | Available authorized agy or another genuinely independent engine | Evaluate grounded alternative proposals; do not assume a model has unique training data |
