...
- **vs `delve`** — delve excavates ONE shipped feature via dialogue for evolution directions (forward, INTERACTIVE); cartograph maps a feature/system across multiple repos autonomously into diagrams + a design doc (backward). Axis: **comprehend-and-document-across-repos** vs **excavate-one-feature-for-evolution**. Full detail: see Decision tree.
- **vs `charter`** — charter plans forward team + work from one repo; cartograph documents backward how a multi-repo system is built today. `cartograph → charter` turns the map into a delivery plan. Full detail: see Decision tree.
...
- **vs `clone`** — clone rebuilds an external product's black-box surface in code, verified by differential parity; cartograph documents your own/accessible white-box source, verified by grounding, writing no code. Full detail: see Decision tree.
- **vs `migrate`** — migrate changes an own-system wholesale (writes code); cartograph only documents the system as-is. `cartograph → migrate` when the map reveals a change worth making. Full detail: see Decision tree.
- **vs `package`** — package generates a document package from a preset registry (forward authoring); cartograph reverse-engineers existing code into diagrams + a design doc (backward, code-grounded). Full detail: see Decision tree.
- **vs `lens` / `atlas` / `canvas` (agents)** — each is single-agent/single-repo (lens comprehends one codebase, atlas analyzes one repo's architecture, canvas draws one diagram); cartograph orchestrates all three plus Scribe + a grounding gate across repos. A single-repo overview → these direct. Full detail: see Decision tree.
