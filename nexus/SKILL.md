...
- Use **Managed Agents** vocabulary and surface an escalation recommendation in `NEXUS_COMPLETE` when the workload justifies the managed platform; prefer **Dynamic Workflows** for large homogeneous parallel sweeps (`reference/managed-agents-mapping.md` §5).
...
- Track orchestration efficiency (OE = successful tasks / compute cost) and token efficiency per chain; split the denominator with `thinking_tokens` where available — over-thinking and over-writing are different problems one total can't distinguish.
...
**Triggers:** `/Nexus` with no arguments → proactive mode (`reference/proactive-mode.md`); `## NEXUS_ROUTING` → hub mode, returning via `## NEXUS_HANDOFF` with no direct agent-to-agent calls. In `AUTORUN`/`AUTORUN_FULL`, execute immediately unless **Ask First** or `confidence-scoring.md` Part 2 requires confirmation.
...
Four families hold genuinely confusable siblings and are disambiguated by an explicit axis: **Improve** (`refactor`/`optimize`/`kaizen`/`anneal`/`restyle`), **Loop** (`goal`/`converge`/`quell` — the axis is *what the completion oracle is*; every loop passes `_common/LOOP_PRECONDITIONS.md` first), **Reproduce, Synthesize & Invent** (`clone`/`fuse`/`graft`/`transmute`/`migrate`/`eureka` — the axis is source count; shared `_common/DIFFERENTIAL_PARITY.md`), and **Quality-Max** (expensive, confirm — one question each). Those axes, the remaining families, and the full prose → `reference/recipes-detail.md` § Recipe Families.

### Recipe Registry

**Full table** → **`reference/recipes-index.md`**; read it once a subcommand matches, or when scanning to pick a Recipe. The list below is the dispatch allowlist only — a token not on it is not a subcommand.
...
Execution-control Mode is applied after Recipe selection (orthogonal). Inline Recipes (`kaizen`, `essential`, `killer`, `trim`) have no top-level reference — contracts in `reference/inline-recipes.md`.
...
**Orchestrator detection** — detect which CLI drives *this hub session* once, before the first spawn (`Agent` → Claude Code; `spawn_agent` → Codex CLI; `/agent` in a TUI main session → agy), then bind the spawn API, authoring protocol, and model map. Detection table, per-CLI spawn APIs and prereqs, model selection, adaptive-prompt policy, and the canonical spawn template → `reference/hub-authoring.md` § Execution Model + `reference/execution-layers.md`.

**Spawn decision** — Core Rule #3 decides: no spawn tool → internal (log the verified blocker); specialist expertise → spawn (mandatory); trivial edit → spawn only if the overhead is justified. Bound the *upper* count, and **never spawn an agent to re-check another's output** — that is a sequential VERIFY step, not a sibling.
...
- **Always confirm:** the triggers in **Boundaries → Ask First**. Full detail → `reference/guardrails.md` § Safety Contract.
...
**Chain source of truth:** `routing-matrix.md` answers "which agents fire for task X" (task type → default chain across **93 types**, classify/LADDER flow, per-task-type phase contracts, Sherpa-skip and chain-adjustment rules) — the Registry exposes the most-used types as subcommands, the rest reach via `classify`. `agent-chains.md` answers "how to adjust a chain" (parallel variants, Rally escalation, addition/skip triggers). `recipes-detail.md` holds Recipe Families axis prose; per-recipe phase contracts live in each `<recipe>-recipe.md`.
...
**Required contracts:**
- `DELIVER` returns `NEXUS_COMPLETE` semantics (`reference/output-formats.md`); `AUTORUN` appends `_STEP_COMPLETE:` with `Agent`, `Status`, `Output`, `Next` after normal work.
...
Read only the files that match the current decision point. Files indexed by the Workflow table's Read-When column (routing-matrix, agent-chains, confidence-scoring, execution-phases, guardrails, error-handling, output-formats, the anti-pattern catalogs) are not repeated — the Workflow row is their index.

| File | Read When |
|------|-----------|
| **`reference/reference-index.md`** | **The full Read-When index for every remaining reference** — loop discipline, verdict/parity/refutation contracts, dialogue and doc-quality protocols, research grounding, proof-carrying, skill packs, image input, managed agents, per-engine hub authoring. Read it whenever the row you need is not below. |
...
| `reference/<recipe>-recipe.md` | Per-Recipe phase contracts, chain templates, cost profiles; filename = the `Read` column of `recipes-index.md` |
