...
| **Reason** (no code) | `gedanken` | structured thought-experiment: construct → reason → perturb → refute → conclude. Output = insight, not a build. Orchestrates `magi`/`flux`/`omen`/`matrix`; trivial "what if" → `flux`/`magi` direct. |
