...
| `reference/research-grounding.md` | Web-research sweep shared by `clone`/`fuse`/`graft` (Phase 0.5, Evidence Ledger) and `eureka` (Phase 1.5, **inverted polarity** — the ledger is a novelty denominator to avoid, not a target to reproduce); `newsroom` reuses the ledger schema + trust tiers (primary-source-authoritative variant) |
