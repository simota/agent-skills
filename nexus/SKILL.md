...
| Loop | `loop` | **Loop-engineering dispatcher & discipline gate** — front-door for *underspecified* "make a loop" requests. Classifies the loop's shape, runs the **loop-engineering precondition gate** (verifiable oracle · external hard-stop · maker≠checker · persistent memory · drift-aware — each failed check maps to a named anti-pattern: loopmaxxing / overbaking / nodding / amnesiac), then **routes to the engine that owns execution** (`goal` / `converge` / `orbit` / `apex`). Delegates, never re-implements `orbit`. Gate is contract-level (AUTORUN cannot skip); unattended `orbit` runs confirm before launch. 1 agent (inline) + routed engine's range. | See `reference/recipes-detail.md` | `reference/recipes-detail.md`, `reference/loop-recipe.md` |
...
| Operating Layer | `layer` | **Design + stand up a repo's operating layer** — the coordinated set of project-local skills + repo-tailored recipes + skill-and-agent workflows + routing map a repository should have. **Loom** designs the blueprint, **Sigil** authors the skill bodies (9+/12), **Nexus** registers recipes/workflows/routing map, Latch/Orbit take hooks/loops. `design-only` stops at the blueprint (charter-style). Confirm-before-launch; `≥10` skills / routing changes → Ask First. 5-15 agents (design-only 3-6). | See `reference/recipes-detail.md` | `reference/recipes-detail.md`, `reference/layer-recipe.md` |
...
| `layer`, `operating layer`, `project skill suite`, `set up the project's agents/recipes/workflows`, `design this repo's tooling`, `repo task playbooks`, `project routing map`, `loom` | `layer` (Loom designs the operating-layer blueprint → Sigil authors bodies → Nexus registers; `design-only` stops at the blueprint. Distinct from `charter` = team+work plan, `sigil` = one skill body) |
