...
| `com.apple.security.automation.apple-events` | Send Apple Events broadly (paired with per-target `NSAppleEventsUsageDescription` prompts at runtime) | Only for automation-hub apps; route "Native builds a scriptable app" vs "app scripts other apps" through the Native↔Anvil boundary — this entitlement is for the latter direction |
