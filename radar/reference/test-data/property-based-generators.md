...
Property-based testing generates many random inputs to verify that properties (invariants) hold. Radar designs the **generators** (arbitraries/strategies) that produce these inputs.

```
Generator → Random Input → Function Under Test → Property Check
                ↑                                      ↓
            Radar's job                           Radar's job
```
...
Radar designs generators; Radar writes properties and assertions.

```yaml
RADAR_FIXTURES_TO_SUITE_HANDOFF:
  generators:
    - name: userGen
