...
- **Radar `pii`**: test-dataset masking algorithms (tokenization / FPE / k-anon / l-div / t-close / DP). Optimized for reproducible test utility with privacy floor.
...
- **Canon[regulatory] (elsewhere)**: regulatory framework mapping (GDPR / HIPAA / PCI-DSS controls, audit trails, Policy as Code).
- **Siege (elsewhere)**: volume generation for load tests; does not mask, consumes masked output.

If the hypothesis is "is this test dataset safe to check in or share with a vendor?" → `pii`. If it's "is our production pipeline GDPR-lawful?" → Cloak + Canon[regulatory].
...
Default for Radar test datasets: **deterministic HMAC tokenization** for identifiers + **Faker replacement** for display fields + **suppression** for Critical-tier (SSN / card PAN / password hash).
...
Typical budgets: ε = 1 (strong), ε = 5 (weak but sometimes unavoidable), ε = 10 (explain to Canon[regulatory]). Track cumulative ε across a dataset's lifetime — each query spends from the budget.
...
**To Siege:** masked dataset + volume multiplier recipe; Siege inflates, Radar masks.
...
**To Canon[regulatory]:** dataset-level evidence package (technique per column, k / l / t / ε values, retention policy) for audit trail.
